...
import { nat, sameValueZero } from "./primitives.js";
import { Equality, Order, Relation } from "./things.js";
...
export function assertT(condition : any) : asserts condition is true {
...
export function assertIsDefinedT<T>(value : T) : asserts value is NonNullable<T> {
    if (value === undefined || value === null) throw new AssertionFailed("undefined value");
}

export function assertIsUndefinedT(value : any) : asserts value is undefined | null {
    if (value !== undefined && value !== null) throw new AssertionFailed("value is defined as '" + value + "'");
}

export function assertEqT<E>(...values : E[]) {
...
export function assertEQT<E>(eq : Equality<E>, ...values : E[]) {
    for (let i = 0; i < values.length; i++) {
        for (let j = 0; j < values.length; j++) {
...
export function assertLESST<E>(order : Order<E>, ...values : E[]) {
    for (let i = 0; i < values.length; i++) {
        for (let j = 0; j < values.length; j++) {
...
export function assertLEQT<E>(order : Order<E>, ...values : E[]) {
    for (let i = 0; i < values.length; i++) {
        for (let j = 0; j < values.length; j++) {
...
export function assertUNRELATEDT<E>(order : Order<E>, ...values : E[]) {
    for (let i = 0; i < values.length; i++) {
        for (let j = 0; j < values.length; j++) {
